...
the existing Xaman-JWT path used elsewhere) — treat it as newer and less
...
- No request-correlation IDs on the wire — `Core` matches a response to the
  right in-flight call using whatever the response already echoes back
  (chat ID, identity, etc.), falling back to FIFO only where nothing to
  match on exists yet. In practice this means concurrent calls of the same
  method type resolve correctly.
