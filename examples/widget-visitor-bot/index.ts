...
async function main() {
  ...
  bot.core.on("reconnecting", (attempt) => console.warn(`[visitor-bot] reconnecting (attempt ${attempt})`));
  ...
  bot.onVisitorRoomRequested((handle, firstMessage) => {
    console.log(`[visitor-bot] room ${handle.roomToken} opened by "${handle.visitorLabel}" from ${handle.origin}`);
  ...
  if (!widget) {
    throw new Error(`No widget found for ${wallet.address} — create one first at ${WEBAPP_URL}/app/widgets`);
  }
