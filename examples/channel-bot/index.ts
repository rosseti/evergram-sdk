...
async function main() {
  ...
  bot.core.on("reconnecting", (attempt) => console.warn(`[channel-bot] reconnecting (attempt ${attempt})`));
  ...
  bot.core.on("visitorChannelParticipantJoined", ({ sender, previousSender }) => {
    console.log(
      previousSender ? `[channel-bot] ${previousSender} is now ${sender}` : `[channel-bot] ${sender} joined the channel`
    );
  });
  ...
  bot.core.on("visitorChannelModeChanged", ({ moderated, ops, voiced }) => {
    console.log(`[channel-bot] mode: moderated=${moderated} ops=[${ops.join(", ")}] voiced=[${voiced.join(", ")}]`);
  });
  ...
  bot.onVisitorMessage(async (msg, handle) => {
    ...
    if (command === "/mod" || command === "/unmod") {
      await handle.moderate(command === "/mod" ? ModerationAction.MODERATION_SET_MODERATED : ModerationAction.MODERATION_UNSET_MODERATED);
      return;
    }
    ...
    if (action !== undefined && target) {
      ...
      if (!(resp as { status?: { ok: boolean } }).status?.ok) {
        handle.reply(`Couldn't ${command.slice(1)} ${target} — are they in the channel and are you an op?`);
      }
      return;
    }
  ...
  if (channelKeyHex.length !== 64) {
    ...
    const config: WidgetConfig = { ...(widget.config ?? {}), mode: "public_group", channelKey: channelKeyHex } as WidgetConfig;
    await bot.core.updateWidgetConfig(widget.widgetId, config);
    console.log(`[channel-bot] widget "${widget.name}" switched to public_group with a fresh channel key`);
  }

  const { status, roomToken, participants } = await bot.core.subscribePublicChannel(widget.widgetId, channelKeyHex);
