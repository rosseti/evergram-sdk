...
function buildVisitorSessionHandle(
  ...
  return {
    ...
    announcePresence: (sender, previousSender) =>
      core.announceChannelPresence(meta.roomToken, sender, previousSender),
    moderate: (action, targetParticipant) =>
      core.moderateChannel(meta.roomToken, action, targetParticipant),
  };
}
...
type ReactionHandler = (
  reaction: EvergramReaction,
  chat: ChatInfo | undefined,
) => void | Promise<void>;
type MessageEditedHandler = (
  edit: EvergramMessageEdited,
  chat: ChatInfo | undefined,
) => void | Promise<void>;
type MessageDeletedHandler = (
  deletion: EvergramMessageDeleted,
  chat: ChatInfo | undefined,
) => void | Promise<void>;
...
type VisitorMessageHandler = (
  msg: EvergramVisitorMessage,
  handle: VisitorSessionHandle | undefined,
) => void | Promise<void>;
type VisitorReactionHandler = (
  reaction: EvergramVisitorReaction,
  handle: VisitorSessionHandle | undefined,
) => void | Promise<void>;
...
type VisitorTypingHandler = (
  event: EvergramVisitorTyping,
  handle: VisitorSessionHandle | undefined,
) => void | Promise<void>;
...
export class EvergramBot {
  ...
  private bindEvent<TArg>(event: string, run: (arg: TArg) => void | Promise<void>): () => void {
    const wrapped = (arg: TArg) => {
      // run(arg) itself can throw synchronously (a plain, non-async handler
      // is valid per the void | Promise<void> signature) — that throw would
      // happen before Promise.resolve(...) is ever reached and escape the
      // .catch() below entirely, defeating the guarantee this comment block
      // promises. The try/catch here covers the sync case; .catch() below
      // still covers a rejected returned promise.
      try {
    ...
    const core = this.core as unknown as {
      on(e: string, l: (arg: TArg) => void): void;
      off(e: string, l: (arg: TArg) => void): void;
    };
  ...
  private bindVisitorEvent<TArg extends { roomToken: string }>(
    ...
    return this.bindEvent<TArg>(event, (arg) => {
      ...
      const handle = meta
        ? buildVisitorSessionHandle(this.core, { roomToken: arg.roomToken, ...meta })
        : undefined;
  ...
  onMessage(handler: MessageHandler): () => void {
    return this.bindEvent<EvergramChatMessage>("message", (msg) =>
      handler(msg, this.core.getChat(msg.chatId)),
    );
  }
  ...
  onGroupInvite(handler: GroupInviteHandler): () => void {
    ...
    const unsubscribe = this.bindEvent<PendingGroupInvite>("groupInviteReceived", (event) =>
      handler(asHandle(event)),
    );
  ...
  onVisitorChannelParticipantJoined(handler: VisitorChannelParticipantJoinedHandler): () => void {
    return this.bindVisitorEvent<EvergramVisitorChannelParticipantJoined>(
      "visitorChannelParticipantJoined",
      handler,
    );
  }
  ...
  onVisitorChannelParticipantLeft(handler: VisitorChannelParticipantLeftHandler): () => void {
    return this.bindVisitorEvent<EvergramVisitorChannelParticipantLeft>(
      "visitorChannelParticipantLeft",
      handler,
    );
  }
  ...
  onVisitorChannelModeChanged(handler: VisitorChannelModeChangedHandler): () => void {
    return this.bindVisitorEvent<EvergramVisitorChannelModeChanged>(
      "visitorChannelModeChanged",
      handler,
    );
  }
