...
function parseModerationStatePayload(payloadText: string): EphemeralModerationState | null {
  try {
    const parsed = JSON.parse(payloadText);
    if (
      typeof parsed?.moderated !== "boolean" ||
      !Array.isArray(parsed?.ops) ||
      !Array.isArray(parsed?.voiced)
    ) {
      return null;
    }
...
export function describeModerationChange(
  ...
  if (prev.moderated !== next.moderated) {
    texts.push(
      next.moderated ? "Channel is now moderated (+m)" : "Channel is no longer moderated (-m)",
    );
  }
  ...
  for (const name of added(prev.ops, next.ops))
    texts.push(`${name} was given operator status (+o)`);
...
export function decryptTextFramePayload(
  symKey: Uint8Array,
  payloadText: string,
): EphemeralTextEvent | null {
  try {
...
export class EphemeralRelaySession {
  ...
  handleFrame(kind: RelayMessageKind, payloadText: string) {
    ...
    if (kind === "left") {
      this.opts.onStatusChange("peer_left", {
        peerLeftDeadline: parsePeerLeftDeadline(payloadText),
      });
      this.opts.onTyping({ isTyping: false });
      return;
    }
  ...
  sendTyping(isTyping: boolean, sender?: string) {
    if (isTyping) {
      if (this.typingTimeout) {
        ...
        this.opts.sendFrame(
          "typing",
          JSON.stringify(sender ? { isTyping: true, sender } : { isTyping: true }),
        );
        this.markActivity();
      }
  ...
  private clearTyping(sender?: string) {
    ...
    this.opts.sendFrame(
      "typing",
      JSON.stringify(sender ? { isTyping: false, sender } : { isTyping: false }),
    );
    this.markActivity();
  }
