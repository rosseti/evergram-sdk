...
import {
  buildTextFramePayload,
  decryptTextFramePayload,
} from "../../src/ephemeral-relay-session.js";
...
async function openVisitorRoom(
  visitor: RawClient,
  widgetId: string,
  visitorLabel: string,
  text: string,
  ...
  visitor.send(
    ClientMessage.create({
      createVisitorRoom: {
        ...
        symKey,
        visitorLabel,
        firstMessagePayload: encodeRelayPayload(payloadText),
      },
    }),
  );
...
describe("widget-visitor chat", () => {
  it("delivers a visitor's opening message to the bot and the bot's reply back to the visitor", async () => {
    ...
    const { roomToken, symKey } = await openVisitorRoom(
      visitor,
      widgetId,
      "Curious Visitor",
      "hello from visitor",
