...
describe.skipIf(!admin)("chat management — group-only (needs EVERGRAM_TEST_ADMIN_SEED)", () => {
  it("updateChatRoles promotes a participant to moderator, visible on both sides", async () => {
    ...
    const founderIdentity = identityKey({
      chainFamily: ChainFamily.XRPL,
    ...
    await waitUntil(
      () => !!founder.core.getChat(chatId)?.meta?.roles?.moderators?.includes(memberIdentity),
    );
    await waitUntil(
      () => !!member.core.getChat(chatId)?.meta?.roles?.moderators?.includes(memberIdentity),
    );
  });
